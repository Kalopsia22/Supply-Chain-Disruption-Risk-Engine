...
## Historical Ground-Truth Validation

The **📜 Historical Validation** tab closes a specific gap: the dataset's `Weather_Severity_Index` is synthetic, and the earlier finding that it doesn't predict delay could be read as "well, of course — it's synthetic." This tab ties every shipment to **real historical weather** via Open-Meteo's Historical Weather API (`archive-api.open-meteo.com`, ERA5/IFS reanalysis, free, no key) for its actual origin port and Order_Date.

- **One API call per origin port** (6 total for this dataset), each covering that port's full date range — not one call per shipment, so it stays trivially within the free tier
- Merges real wind speed, wind gusts, and precipitation onto each shipment by (port, date)
- Recomputes correlation with actual delay and compares it directly to the synthetic index
- Optional **model uplift test**: retrains the classifier with real weather features added and reports the ROC-AUC change, so you can see empirically whether real ground truth adds predictive value beyond what's already captured
- Everything is tested end-to-end with mocked responses shaped exactly like Open-Meteo's schema (merge integrity, correlation math, and the retraining/uplift pipeline all verified) — only the live network round-trip itself needs verifying after deployment, for the same sandbox-restriction reason as the Live Port Conditions tab
